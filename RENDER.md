# Render Deploy: N Hermes Agents on Render

One baked image → N independent Render services, one agent each. Per-agent secrets are read from the **environment** (Hermes has no config keys for tokens/keys/allowlists); only the proxy base URL and personality are written to config. Render is auto-detected via the `RENDER` env var.

## Architecture

| Layer | Detail |
|---|---|
| Base image | Single Docker build from this repo. All services share the same image. |
| Per-agent diff | `AGENT_NAME` selects `agents/<AGENT_NAME>/soul.md`. Service env vars inject token, allowlist, and keys at boot. |
| Storage | Render persistent disk mounted at `/data`. Each agent scopes under `/data/<AGENT_NAME>/`. |
| Respawn | `entrypoint.sh` auto-restarts the gateway on crash. |

## Environment variables (service env / env groups)

| Variable | Required | Description |
|---|---|---|
| `AGENT_NAME` | Yes | Matches `agents/<AGENT_NAME>/soul.md`. Stable per service — never change after first deploy (it scopes `/data/<AGENT_NAME>/`; changing it orphans all state). |
| `TELEGRAM_BOT_TOKEN` | Yes | Unique Telegram bot token (one per agent, from BotFather). |
| `TELEGRAM_ALLOWED_USERS` | Yes | Comma-separated Telegram user IDs allowed to DM the bot. |
| `GEMINI_API_KEYS` | Yes* | JSON list of Gemini keys, e.g. `["AIza...1","AIza...2"]`. Reset + re-seeded into a round-robin pool every boot. |
| `GEMINI_API_KEY` | Yes* | Single-key fallback if `GEMINI_API_KEYS` is unset. |
| `TELEGRAM_HOME_CHANNEL` | No | Chat ID for cron/notification delivery. |
| `TELEGRAM_BASE_URL` | No | Telegram API proxy base URL. Defaults to the committed Cloudflare Worker. |
| `AGENT_PERSONALITY` | No | Override `display.personality` (default `kawaii`). |
| `HF_TOKEN` | No | Hugging Face token (for model access). |

\* Provide either `GEMINI_API_KEYS` (preferred) or `GEMINI_API_KEY`.

Share secrets across services with a Render **env group**; set per-agent vars (`AGENT_NAME`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ALLOWED_USERS`) on the individual service.

## Deploy agent N

1. **Create the soul:** add `agents/<AGENT_NAME>/soul.md` (persona) and optional `agents/<AGENT_NAME>/agent.env` (non-secret overrides) in this repo, then push.

2. **New Render service** → **Web Service** from this repo, runtime **Docker** (uses the repo `Dockerfile`).

3. **Add a persistent disk** mounted at `/data`.

4. **Set env vars** — every required variable from the table above (use an env group for shared secrets).

5. **Health check** — Render injects its own `PORT`; `entrypoint.sh` binds the dummy HTTP server to `${PORT:-7860}`, so the health check works without setting a port. (HF/local default to `7860`.)

6. **Deploy.** Render builds the image and boots the agent.

## Important notes

- **Stable AGENT_NAME:** never change it for an existing service. State lives at `/data/<AGENT_NAME>/`; a rename orphans sessions, memories, and the credential pool.
- **Gemini pool is env-driven:** keys are reset and re-added from `GEMINI_API_KEYS` on every boot, so editing the var + redeploying updates the pool exactly. No stale keys accumulate.
- **Tokens/allowlists are env-only:** Hermes reads `TELEGRAM_BOT_TOKEN` / `TELEGRAM_ALLOWED_USERS` / `TELEGRAM_HOME_CHANNEL` from the environment — they are NOT written via `hermes config set`.
- **Telegram proxy:** `TELEGRAM_BASE_URL` is set into `gateway.platforms.telegram.extra.base_url`; the optional `TELEGRAM_PROXY_HOST` sed-patch is also applied if set.
- **tmate SSH:** the entrypoint starts tmate on every platform and prints the SSH URL to the service logs — open the Render log stream to grab it.
- **Token rotation:** rotate via BotFather, update the service env var, redeploy.
- **Respawn on crash:** the entrypoint loops forever, restarting the gateway on any non-zero exit.
